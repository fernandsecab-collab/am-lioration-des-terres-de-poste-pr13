...
function targetInterconnected(regime){ return String(regime||'').trim() === '1000 A' ? 1 : 2.5; }
...
function compute(m){
  const input=diagnosticInputSnapshot(m);
  const {rm,rng,rni,rmn,rcDirect:rcD}=input;
  if(input.terreConfig==='interconnectee'){
    const target=targetInterconnected(input.regime); const ok=atMost(rng,target);
    return {rm,rng,rni,rmn,rc:NaN,c:NaN,target,ok,mode:'interconnectee',fingerprint:diagnosticFingerprint(m),diagnostic:!Number.isFinite(rng)?'RNg à renseigner.':ok?'Prise de terre globale conforme.':'Prise de terre globale à améliorer.',initial:rng};
  }
  const rcRaw = input.mode==='direct' ? rcD : (Number.isFinite(rm)&&Number.isFinite(rni)&&Number.isFinite(rmn)?(rm+rni-rmn)/2:NaN);
  const rc=stable(rcRaw,6);
  const cRaw = Number.isFinite(rm)&&rm>0&&Number.isFinite(rc)?rc/rm:NaN;
  const c=stable(cRaw,6);
  const ok=Number.isFinite(c)&&atMost(c,COUPLING_LIMIT)&&rc>=-ELECTRICAL_EPSILON;
  const issues=[];
  if(input.mode==='edf' && (!Number.isFinite(rni)||!Number.isFinite(rmn))) issues.push('RNi et RMN sont obligatoires pour le calcul EDF complet.');
  if(Number.isFinite(rni)&&Number.isFinite(rng)&&rni<=rng) issues.push('Vérifier RNi > RNg lorsque RNg est connu.');
  if(Number.isFinite(rm)&&Number.isFinite(rni)&&Number.isFinite(rmn)&&rm+rni+ELECTRICAL_EPSILON<rmn) issues.push('RM + RNi doit être supérieur ou égal à RMN.');
  return {rm,rng,rni,rmn,rc,c,target:COUPLING_LIMIT,ok,mode:'separee',fingerprint:diagnosticFingerprint(m),issues,diagnostic:!Number.isFinite(c)?'Mesures à compléter.':ok?'Couplage conforme.':'Couplage non conforme : c > 0,15.',initial:Number.isFinite(rm)?rm:NaN};
}
...
function solutionTechnicalJustification(m,sol){
  const c=compute(m), estimate=sol?estimatedCouplingForSolution(m,sol):{value:NaN,estimatedRc:NaN};
  const final=computeFinal(m), target=sol?.target==='neutral'?'terre du neutre':'terre des masses';
  const ref=sol?.id?.startsWith('h61')
    ? 'Mémento EDF B13-23 — prescriptions de réalisation des prises de terre H61 et schémas multidirectionnels.'
    : 'Mémento EDF B13-23 — page 8, formes de prises de terre et dispositions de mise en œuvre associées.';
  if(!sol)return {
    simple:'Aucune nouvelle solution n’est proposée : l’installation existante est conservée parce que les mesures enregistrées satisfont les critères renseignés dans le dossier. Cette conclusion reste liée aux mesures affichées dans le rapport et à leur cohérence.',
    professional:`Décision fondée sur le calcul reproductible du moteur et sur les seuils renseignés pour l’ouvrage. Référence : ${ref}`,
    reference:ref,
    validation:'La conformité demeure conditionnée à la validité des mesures et, lorsqu’il y a travaux, au contrôle final réalisé sur site.'
  };
  const geometry=sol.id==='patte'||sol.id==='h61-10'||sol.id==='h61-5'
    ? 'La géométrie multidirectionnelle augmente la longueur de conducteur en contact avec le sol et répartit le courant de défaut dans plusieurs directions, ce qui améliore la dispersion et limite la concentration du potentiel autour de l’ouvrage.'
    : sol.id.includes('serp')||sol.id.includes('grille')||sol.id==='bff'
      ? 'La longueur de conducteur enterré et sa répartition dans la tranchée augmentent la surface d’échange avec le terrain, ce qui améliore la dispersion du courant de défaut et réduit la résistance de la prise traitée.'
      : 'L’électrode verticale atteint des couches de sol différentes et augmente la surface conductrice utile, ce qui améliore la dispersion du courant de défaut avec une faible emprise au sol.';
  const electrical=sol.target==='neutral'
    ? `Le déplacement ou l’amélioration de la terre du neutre vise à réduire l’influence électrique commune avec la terre des masses ; l’estimation porte le coefficient de ${fmt(c.c,3)} à ${fmt(estimate.value,3)}, sous réserve de mesurer après travaux RM, RNi et RMN.`
    : `L’amélioration de la ${target} vise une valeur simulée de ${fmt(sol.after,2)} Ω ; son effet réel sur le coefficient de couplage doit être contrôlé après travaux, car une diminution de RM seule ne garantit pas automatiquement une baisse de c = Rc/RM.`;
  return {
    simple:`${geometry} ${electrical}`,
    professional:`La solution est retenue parce que sa forme, son emprise et sa longueur correspondent aux dispositions représentées dans le ${ref} Le calcul est une aide à la décision : la conformité définitive repose sur les mesures finales et sur un coefficient de couplage inférieur ou égal à 0,15 lorsqu’il est applicable.`,
    reference:ref,
    validation:Number.isFinite(final.c)?`Mesure finale enregistrée : c = ${fmt(final.c,3)} (${atMost(final.c,COUPLING_LIMIT)?'conforme au seuil ≤ 0,15':'non conforme au seuil ≤ 0,15'}).`:'La solution est prévisionnelle tant que les mesures finales RM, RNi et RMN ne sont pas enregistrées.'
  };
}
...
function technicalReferences(m,sol){return [
  {title:'Mémento EDF B13-23',version:m.referenceVersion||'Version à confirmer dans le dossier documentaire',scope:sol?.id?.startsWith('h61')?'Dispositions applicables aux prises de terre H61':'Formes de prises de terre et principes de mise en œuvre',location:sol?.id?.startsWith('h61')?'Chapitre H61 / schéma applicable':'Page 8 — géométries de prises de terre',status:'Référence de conception ; ne remplace pas les mesures finales'},
  {title:'Règles de calcul Mesure & amélioration des terres de poste de transformation',version:'Moteur déterministe V2.0',scope:'Contrôle de cohérence, calcul Rc, coefficient, classement multicritère et traçabilité',location:'Règles internes versionnées',status:'Aide à la décision auditable'}
]}
...
function expertiseAssessment(m,sol){
  ...
  const scores={
    technical:sol?Math.max(40,Math.min(99,70+Number(sol.eff||5)*2+(Number.isFinite(margin)&&margin>0?10:-8))):(c.ok?96:55),
    ...
    robustness:sol?Math.max(40,Math.min(98,62+Number(sol.eff||5)*3-(Number(m.resistivite||0)>1000?12:0))):(c.ok?94:50),
  ...
  return {scores,weights,overall,level,margin,rejected,limits,quantities:qty,costTotal:costs.total,reason:sol?'La solution retenue obtient le meilleur score pondéré entre performance électrique, coût saisi, durée estimée, impact sur le terrain, facilité d’exécution, robustesse, preuves disponibles et maintenabilité.':'Aucune intervention n’est retenue car les mesures satisfont les critères enregistrés et aucune correction n’est justifiée.',trace:{software:`Mesure & amélioration des terres de poste de transformation V${APP_VERSION}`,engine:'Moteur déterministe et multicritère — règles V2.0',fingerprint:diagnosticFingerprint(m),generated:new Date().toLocaleString('fr-FR'),source:'Mémento EDF B13-23 + référentiel documentaire versionné + données de l’affaire',cartography:m.mapProvider||'IGN / Géoportail prioritaire',coordinates:`${m.gpsLat||'—'} / ${m.gpsLng||'—'} (précision ${m.gpsAccuracy||'—'} m)`,operator:m.technicien||'Non renseigné'}};
}
...
function estimatedCouplingForSolution(m,sol){
  ...
  if(sol?.id==='ultimate-decoupling'){
    const rcLimit=COUPLING_LIMIT*c.rm;
    const estimatedRc=Math.min(c.rc*.20,rcLimit*.75);
    const value=estimatedRc/c.rm;
    return {value,estimatedRc,label:'Objectif estimé atteignable par découplage renforcé',assumption:'Procédure renforcée : suppression des liaisons parasites, déplacement de la MALT du neutre hors zone d’influence commune et création d’une prise dédiée. Cette estimation sert à la planification ; seules les mesures finales RM, RNi et RMN valident c < 0,15.'};
  }
  if(sol?.target==='masses'){
    ...
    return {value,estimatedRc,label:Number.isFinite(value)?(atMost(value,COUPLING_LIMIT)?'Objectif théorique atteint':'Objectif non atteint'):'Non calculable',assumption:'Hypothèse conservatrice : Rc reste inchangée. Une baisse de RM seule peut dégrader le coefficient.'};
  }
  ...
  const value=estimatedRc/c.rm;
  return {value,estimatedRc,label:atMost(value,COUPLING_LIMIT)?'Objectif estimé atteignable':'Amélioration estimée insuffisante',assumption:'Estimation de planification basée sur l’efficacité indicative, la baisse théorique de RNi et l’éloignement du nouvel ouvrage. Elle ne remplace jamais les mesures finales RM, RNi et RMN.'};
}
function ultimateDecouplingSolution(m){
  const c=compute(m), target=couplingTargetPlan(m);
  const rm=Number.isFinite(c.rm)?c.rm:NaN;
  const rcMax=Number.isFinite(rm)?0.15*rm:NaN;
  const estimatedRc=Number.isFinite(rcMax)?rcMax*.75:NaN;
  const after=Number.isFinite(c.rni)?Math.max(.5,c.rni*.35):NaN;
  return {
    ...
    electrode:NaN,
    after,
    gain:Number.isFinite(c.rni)&&Number.isFinite(after)?c.rni-after:NaN,
...
function rankedApplicableSolutions(m){
  ...
  if(c.mode==='separee'&&Number.isFinite(c.c)&&c.c>COUPLING_LIMIT+ELECTRICAL_EPSILON){
    const compliant=candidates.filter(sol=>solutionMeetsCouplingTarget(m,sol));
    if(compliant.length)return compliant.map((sol,i)=>({...sol,recommended:i===0,couplingQualified:true}));
    if(!['incomplete','reference-required','neutral-blocked','review'].includes(strategy.kind))return [ultimateDecouplingSolution(m)];
    return [];
  }
  return candidates.sort((a,b)=>(Number(b.recommended)-Number(a.recommended))||(Number(a.work)-Number(b.work))||(Number(b.eff)-Number(a.eff))||String(a.id).localeCompare(String(b.id)));
}
...
function canonicalSolutionPatch(m,solutionId,extra={}){
  ...
  return {
    ...
    implantation:{...implantation,selectedSolution:id,solutionSnapshot:{id,title:(rankedApplicableSolutions(m).find(x=>x.id===id)||ELECTRODES.find(x=>x.id===id))?.title||m?.implantation?.solutionSnapshot?.title||''},...(extra.implantation||{})},
...
function implantationAnchor(m){
  const neutral=m?.improvementTarget==='neutral', source=sourceGpsAnchor(m), imp=m?.implantation||{};
...
function analyzeImplantation(m,solution,angle){
  ...
  // La liaison doit arriver exactement sur le nœud électrique réel du schéma.
  // Pour les formes à branches (patte d'oie, H61, déplacement du neutre), ce nœud
  // est l'intersection commune des branches, donc le point local [offsetX, offsetY].
  const connectionTarget=toLngLat(offsetX,offsetY,lng,lat);
  const connectorGeo=(Number.isFinite(Number(baseLat))&&Number.isFinite(Number(baseLng))&&distanceMeters(Number(baseLng),Number(baseLat),connectionTarget[0],connectionTarget[1])>.05)
    ? [[[Number(baseLng),Number(baseLat)],connectionTarget]]
    : [];
...
function Solutions({m,update,next,nextNoWork,back}){
  ...
  {!noWork&&!decisionBlocked&&sols.length>0&&<div className="card rankedSolutions"><div className="rankedSolutionsHead"><div><h2>Solutions validées par la simulation — c &lt; 0,150</h2><p>Seules les solutions dont le coefficient estimé après travaux est strictement inférieur à 0,150 sont affichées. Les autres sont automatiquement masquées.</p></div><span>{strategy.target==='neutral'?'Action automatique : terre du neutre':'Action automatique : terre des masses'}</span></div><div className="solutionGrid">{sols.map((sol,i)=><article key={sol.id} className={`solution optionSolution ${chosenId===sol.id?'selectedSolution':''}`}><div className="solutionHead"><span>#{i+1} · Niveau travaux {sol.work}/8</span><b>{sol.title}</b></div><ReferenceDiagram solutionId={sol.id} title={sol.title}/>{sol.recommended&&<div className="recommendationBadge">Meilleur point de départ</div>}{sol.conditional&&<div className="conditionalBadge">Après contrôle / prescription</div>}{sol.couplingQualified&&<div className="couplingQualifiedBadge">✓ Simulation c &lt; 0,150</div>}{sol.ultimate&&<div className="ultimateSolutionBadge">PROCÉDURE RENFORCÉE DE DERNIER RECOURS</div>}<p><b>Emprise :</b> {sol.footprint}</p><p><b>Matériel :</b> {sol.material}</p><ol>{sol.steps.map(x=><li key={x}>{x}</li>)}</ol><div className="values"><span>Niveau de travaux <b>{sol.work}/8</b></span><span>Efficacité indicative <b>{sol.eff}/10</b></span><span>Résistance théorique de l’électrode ajoutée <b>{fmt(sol.electrode,2)} Ω</b></span><span>{sol.target==='neutral'?'RNi théorique après ajout':'RM théorique après ajout'} <b>{fmt(sol.after,2)} Ω</b></span><span>Coefficient estimé <b>{fmt(estimatedCouplingForSolution(m,sol).value,4)}</b></span><span>Coefficient cible <b>&lt; 0,150</b></span><span>Rc cible <b>{fmt(couplingTargetPlan(m)?.rcMax,3)} Ω</b></span></div><SolutionPerformancePanel m={m} sol={sol}/><VegetalEarthPanel m={m} sol={sol}/>{sol.note&&<p className="solutionNote">{sol.note}</p>}<button type="button" className={chosenId===sol.id?'selected':''} onClick={()=>selectSolution(sol)}>{chosenId===sol.id?'✓ Solution retenue':'Choisir cette solution'}</button></article>)}</div></div>}
  {!noWork&&chosen&&<><article className="solution chosen autoDecision"><div className="solutionHead"><span>Solution retenue · {chosen.target==='neutral'?'Terre du neutre':'Terre des masses'}</span><b>{chosen.title}</b></div>{chosen.ultimate&&<div className="ultimateWarning"><b>Aucune prise de terre classique simulée n’atteint c &lt; 0,150.</b><span>Cette procédure traite directement la cause du couplage. Elle exige une validation EDF SEI avant travaux et une nouvelle campagne de mesures après exécution.</span></div>}{chosen.id.startsWith('h61-')?<H61Diagram longBranch={chosen.id==='h61-5'?5:10} central={chosen.id==='h61-5'?3:5}/>:<ReferenceDiagram solutionId={chosen.id} title={chosen.title}/>}<p><b>Emprise :</b> {chosen.footprint}</p><p><b>Matériel :</b> {chosen.material}</p><ol>{chosen.steps.map(x=><li key={x}>{x}</li>)}</ol><div className="values"><span>Action ciblée <b>{chosen.target==='neutral'?'Terre du neutre':'Terre des masses'}</b></span><span>Rc à atteindre <b>≤ {fmt(couplingTargetPlan(m)?.rcMax,3)} Ω</b></span><span>Coefficient cible <b>&lt; 0,150</b></span><span>Coefficient final <b>À recalculer après RM, RNi et RMN finales</b></span>{chosen.requiresApproval&&<span>Validation EDF SEI <b>Obligatoire</b></span>}</div><SolutionPerformancePanel m={m} sol={chosen} detailed/><VegetalEarthPanel m={m} sol={chosen} detailed/></article><div className="card chosenSolutionCard"><h2>Solution retenue</h2><h3>{chosen.title}</h3><textarea value={m.diagnosticTerrain||''} onChange={e=>update({diagnosticTerrain:e.target.value})} placeholder="Observations complémentaires du technicien"/><textarea value={m.reprise||''} onChange={e=>update({reprise:e.target.value})} placeholder="Travaux à réaliser"/></div><PlanAction m={m} update={update} field="surfaceWorks" title="Plan d’action prévisionnel et quantités de réfection avant travaux"/></>}
...
function DesktopCosting({m,update}){
...
function Terrain({m,update,next,back}){
  ...
  return <section className="grid two">
    <div className="card"><h2>Terrain / GPS</h2>
      ...
      {gpsReady&&<div className="terrainInlineMap"><div className="terrainMapHeader"><div><h3>Géolocalisation du poste</h3><p>Le GPS place d’abord le poste. Glissez ensuite le symbole électrique pour le positionner exactement sur l’ouvrage réel.</p></div><div className="terrainHeaderActions"><span className={`mapStatusBadge ${m.cadastreSync?.status||'idle'}`}>{m.cadastreSync?.status==='ready'?'Données prêtes':m.cadastreSync?.status==='loading'?'Chargement…':'Position GPS'}</span>{m.gpsOriginalLat&&m.gpsOriginalLng&&<button type="button" className="secondaryAction" onClick={()=>update({gpsLat:m.gpsOriginalLat,gpsLng:m.gpsOriginalLng,gpsManuallyAdjusted:false,implantation:{...(m.implantation||{}),centerLat:m.gpsOriginalLat,centerLng:m.gpsOriginalLng}})}>Recentrer sur le GPS</button>}</div></div><LiveMap m={{...m,improvementTarget:'masses',implantation:{...(m.implantation||{}),centerLat:m.gpsLat,centerLng:m.gpsLng,manualAnchorLat:'',manualAnchorLng:''}}} analysis={{lines:[],parcels:[]}} showCadastre={false} showArcgis={true} anchorDraggable={true} onMoveAnchor={(lat,lng)=>update({gpsLat:lat.toFixed(7),gpsLng:lng.toFixed(7),gpsManuallyAdjusted:true,implantation:{...(m.implantation||{}),centerLat:String(lat),centerLng:String(lng)}})}/><div className="terrainMapFooter"><span>⚡ Poste déplaçable librement</span><span>Précision GPS : {m.gpsAccuracy||'—'} m</span><span>{m.gpsManuallyAdjusted?'Position corrigée manuellement':'Position GPS d’origine'}</span></div></div>}
...
function NeutralTargetLocationMap({m,update}){
  ...
  return <div className="neutralLocationMapCard"><div className="neutralMapHeader"><div><h3>Carte du 2ᵉ ouvrage</h3><p>{m.neutralTargetLabel||'2ᵉ ouvrage neutre'} · géolocalisez puis déplacez librement le symbole jusqu’à sa position exacte</p></div><div className="neutralHeaderActions"><b>{Number.isFinite(distance)?`${fmt(distance,1)} m du poste`:'Distance au poste à calculer'}</b>{m.neutralGpsOriginalLat&&m.neutralGpsOriginalLng&&<button type="button" className="secondaryAction" onClick={()=>update({neutralGpsLat:m.neutralGpsOriginalLat,neutralGpsLng:m.neutralGpsOriginalLng,neutralGpsManual:false,neutralMapFocusNonce:Date.now()})}>Recentrer sur le GPS</button>}</div></div><LiveMap key={`neutral-${m.neutralMapFocusNonce||m.neutralGpsCapturedAt||`${lat}-${lng}`}`} focusNonce={m.neutralMapFocusNonce||m.neutralGpsCapturedAt} preferredZoom={18} m={targetRecord} analysis={{lines:[],parcels:[]}} showCadastre={false} showArcgis={false} anchorDraggable={true} onMoveAnchor={(newLat,newLng)=>update({neutralGpsLat:String(newLat.toFixed(7)),neutralGpsLng:String(newLng.toFixed(7)),neutralGpsManual:true})}/><div className="neutralMapCoordinates"><span>Latitude <b>{fmt(lat,7)}</b></span><span>Longitude <b>{fmt(lng,7)}</b></span><span>Position <b>{m.neutralGpsManual?'Corrigée manuellement':'Issue du GPS'}</b></span><span>Fond de carte <b>Plan IGN / orthophoto / OSM</b></span></div></div>
}

function LiveMap({m,analysis={lines:[],parcels:[],connector:[]},onMoveCenter,onMoveAnchor,onParcelSelect,showCadastre=true,showArcgis=true,editable=false,schemaOnly=false,anchorDraggable=false,annotations=[],onAnnotationsChange,onSelectionChange,focusNonce='',preferredZoom=18}){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      L.control.layers({'Plan IGN (Géoportail)':ignPlan,'Orthophoto IGN (Géoportail)':ignOrtho,'Google Satellite':googleSatellite,'Google Hybride':googleHybrid,'Satellite HD Esri':satellite,'OpenStreetMap':osm},{'Parcelles cadastrales':groups.cadastre,'Réseaux existants / contraintes':groups.networks,'Implantation métier':groups.implantation,'Annotations et mesures':groups.annotations,'Photos géolocalisées':groups.photos},{position:'topright',collapsed:true}).addTo(map);
  ...
  useEffect(()=>{
    const map=mapRef.current,g=groupsRef.current;if(!map||!g.cadastre)return;try{setMapError('');Object.values(g).forEach(x=>x.clearLayers());
      const anchor=implantationAnchor(m),baseLat=anchor.valid?anchor.lat:m.gpsLat,baseLng=anchor.valid?anchor.lng:m.gpsLng,lat=num(m.implantation?.centerLat||baseLat),lng=num(m.implantation?.centerLng||baseLng);
      {const markerLat=Number.isFinite(Number(baseLat))?Number(baseLat):Number(lat),markerLng=Number.isFinite(Number(baseLng))?Number(baseLng):Number(lng);if(Number.isFinite(markerLat)&&Number.isFinite(markerLng)){const anchorMovable=Boolean((anchorDraggable||(editable&&schemaOnly))&&m.implantation?.anchorLocked!==true);const gpsText=anchor.manuallyAdjusted?'Position corrigée manuellement':Number.isFinite(Number(anchor.accuracy))?`GPS · précision ${Math.round(Number(anchor.accuracy))} m`:'Position GPS';const transformerSvg='<svg viewBox="0 0 64 58" aria-hidden="true"><path d="M32 3 61 54H3Z" fill="#ffd400" stroke="#111827" stroke-width="4"/><path d="M34 12 24 32h9l-4 14 14-22h-9l5-12Z" fill="#111827"/></svg>';const marker=L.marker([markerLat,markerLng],{draggable:anchorMovable,autoPan:true,autoPanPadding:[70,70],autoPanSpeed:14,riseOnHover:true,keyboard:false,dragging:true,title:anchorMovable?'Déplacer précisément le poste transformateur':'Poste transformateur verrouillé',zIndexOffset:1600,icon:L.divIcon({className:`secabTransformerMarker ${anchorMovable?'movable':'locked'}`,html:`<div class="secabElectricalSymbol">${transformerSvg}</div><div class="secabMapMarkerLabel"><b>${anchor.type==='neutral'?'2ᵉ OUVRAGE':'POSTE TRANSFO'}</b><span>${gpsText}</span></div>`})}).addTo(g.implantation);marker.bindPopup(`<div class="secabAnchorPopup"><b>${htmlEsc(anchor.label)}</b><span>${anchorMovable?'Glissez le symbole électrique pour placer exactement l’ouvrage. La position GPS d’origine reste conservée.':'Position verrouillée.'}</span></div>`);marker.bindTooltip(`${htmlEsc(anchor.label)} · ${htmlEsc(gpsText)}`,{direction:'top',offset:[0,-30]});if(anchorMovable){let dragTip=null;marker.on('dragstart',()=>{map.dragging.disable();marker.setZIndexOffset(2600);dragTip=L.tooltip({permanent:true,direction:'top',offset:[0,-46],className:'secabPrecisionDragTip'}).setLatLng(marker.getLatLng()).setContent('Déplacement précis…').addTo(map)});marker.on('drag',e=>{const p=e.target.getLatLng();dragTip?.setLatLng(p).setContent(`<b>${p.lat.toFixed(7)} / ${p.lng.toFixed(7)}</b><span>Relâchez pour enregistrer</span>`)});marker.on('dragend',e=>{map.dragging.enable();marker.setZIndexOffset(1600);if(dragTip){map.removeLayer(dragTip);dragTip=null}const raw=e.target.getLatLng();const candidates=[];const addGeom=geom=>{if(!geom)return;if(geom.type==='Point')candidates.push(L.latLng(geom.coordinates[1],geom.coordinates[0]));else if(['LineString','MultiPoint'].includes(geom.type))geom.coordinates.forEach(c=>candidates.push(L.latLng(c[1],c[0])));else if(['Polygon','MultiLineString'].includes(geom.type))geom.coordinates.flat().forEach(c=>candidates.push(L.latLng(c[1],c[0])));else if(geom.type==='MultiPolygon')geom.coordinates.flat(2).forEach(c=>candidates.push(L.latLng(c[1],c[0])))};[...(m.cadastre?.features||[]),...(m.contraintes?.features||[]),...(m.arcgisData?.features||[])].forEach(f=>addGeom(f.geometry));let finalPos=raw,bestPx=10;candidates.forEach(c=>{const d=map.latLngToContainerPoint(c).distanceTo(map.latLngToContainerPoint(raw));if(d<bestPx){bestPx=d;finalPos=c}});marker.setLatLng(finalPos);if(onMoveAnchor)onMoveAnchor(Number(finalPos.lat.toFixed(7)),Number(finalPos.lng.toFixed(7)));else onMoveCenter?.(Number(finalPos.lat.toFixed(7)),Number(finalPos.lng.toFixed(7)))})}}}
...
function Implantation({m,update,next,back}){
  ...
  const moveAnchor=(lat,lng)=>{if(!Number.isFinite(Number(lat))||!Number.isFinite(Number(lng)))return;patchImp({manualAnchorLat:String(Number(lat).toFixed(7)),manualAnchorLng:String(Number(lng).toFixed(7)),placementConfirmed:false});};
  ...
  return <section className="implantationPhase2">
    ...
    <div className="implantationWorkspace">
      <div className="card implantationMapCard">
        ...
        <LiveMap key={`implantation-${anchorKey(m)}-${selected}-${mapReload}`} focusNonce={`${anchorKey(m)}-${mapReload}`} preferredZoom={19} m={{...m,implantation:{...(m.implantation||{}),centerLat:String(centerLat||''),centerLng:String(centerLng||'')}}} analysis={analysis} onMoveCenter={moveCenter} onMoveAnchor={moveAnchor} onParcelSelect={setSelectedParcel} showCadastre={true} showArcgis={false} editable={true} schemaOnly={true}/>
...
function Administration({records,setRecords,driveConfig,authSession,uiTheme,setUiTheme}){
...
function Report({m}){
  const printReport=()=>{const refs=officialReferenceGate();if(!refs.ready){alert('Rapport officiel bloqué : complétez toutes les références documentaires dans Administratif.');return}setTimeout(()=>window.print(),80)};
  ...
  return <section className="reportWorkspace premiumReportWorkspace">
    <div className="reportTopbar noPrint"><div><h2>Rapport final premium SECAB · RC20</h2><p>Le rapport client présente le diagnostic, la solution, le plan et les preuves. Les coûts, durées, équipes et estimations chantier restent strictement internes.</p></div><div className="reportModeChoice"><button className="selected" onClick={()=>{}}>Rapport client</button></div><div className="actions"><button onClick={printReport}>PDF client</button><button onClick={()=>buildPortfolioPdf(m)}>Portfolio client</button><button onClick={()=>buildWordReport(m)}>Word client</button></div></div>
    <div className="reportPages premiumReportPages">
      <article className="a4ReportPage premiumCoverPage">
        ...
        <div className="premiumCoverTop"><div className="premiumCoverBrand"><img src="./secab-logo.jpg"/><div><b>SECAB</b><span>COUPLAGE EXPERT · V2.0</span></div></div><span className={`premiumStatus ${final.ok?'ok':'ko'}`}>{final.ok?'CONFORME':'NON CONFORME'}</span></div>
        <div className="premiumCoverHero">{coverPhoto?.data?<img src={coverPhoto.data}/>:<div className="premiumCoverFallback"><b>PRISES DE TERRE & COUPLAGE</b><span>Dossier technique de contrôle final</span></div>}<div className="premiumCoverOverlay"><small>RAPPORT FINAL</small><h1>SÉCURISER AUJOURD’HUI,<br/><em>PROTÉGER DEMAIN.</em></h1><p>Mesure et amélioration des prises de terre · Poste de transformation HTA/BT</p></div></div>
      ...
      <article className="a4ReportPage premiumReportPage">
        ...
        <section className="beforeAfterVisual">
          ...
          <div className={`beforeAfterCard ${Number.isFinite(perf?.estimatedCoeff)&&atMost(perf.estimatedCoeff,COUPLING_LIMIT)?'ok':'warn'}`}><small>APRÈS SIMULÉ</small><strong>{Number.isFinite(perf?.estimatedCoeff)?fmt(perf.estimatedCoeff,3):fmt(perf?.simulated,2)+' Ω'}</strong><span>Estimation de planification</span></div>
        ...
        <section className="expertiseDecisionBlock">
          <div className="expertiseHeader"><div><span>MOTEUR D’EXPERTISE EXPLICABLE</span><h3>Pourquoi cette solution est classée première</h3></div><strong>{expertise.overall}%<small>confiance {expertise.level.toLowerCase()}</small></strong></div>
      ...
      <article className="a4ReportPage premiumReportPage premiumVisualSummaryPage">
        <PageHeader title="Schémas et dossier photographique" subtitle="Contrôle visuel de la solution, de l’implantation et des preuves terrain" page={7}/>
        <section className="reportVisualSchemaBlock">
          ...
          {sol?<TechnicalSchema m={m} sol={sol} editable={false} compact/>:<div className="reportFallbackSchema"><svg viewBox="0 0 720 260" role="img" aria-label="Schéma de principe de prise de terre"><rect width="720" height="260" rx="18" fill="#f3f6f8"/><rect x="54" y="58" width="130" height="104" rx="10" fill="#0c315c"/><text x="119" y="113" textAnchor="middle" fill="white" fontSize="18" fontWeight="800">OUVRAGE</text><line x1="184" y1="110" x2="332" y2="110" stroke="#d61f3c" strokeWidth="10"/><circle cx="350" cy="110" r="16" fill="white" stroke="#d61f3c" strokeWidth="7"/><line x1="350" y1="126" x2="350" y2="205" stroke="#d61f3c" strokeWidth="8"/><line x1="300" y1="205" x2="400" y2="205" stroke="#d61f3c" strokeWidth="8"/><line x1="315" y1="222" x2="385" y2="222" stroke="#d61f3c" strokeWidth="7"/><line x1="330" y1="239" x2="370" y2="239" stroke="#d61f3c" strokeWidth="6"/><text x="500" y="103" fill="#15324f" fontSize="20" fontWeight="800">Aucune intervention retenue</text><text x="500" y="135" fill="#52677c" fontSize="15" textAnchor="middle">Schéma de principe de l’installation existante</text></svg></div>}
        </section>
